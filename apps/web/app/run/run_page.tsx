  ...
  return (
    <main style={{ maxWidth: 640, margin: "0 auto", padding: "28px 18px 60px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 26 }}>
        ...
        <span style={{ fontSize: 10, letterSpacing: "3px", textTransform: "uppercase", color: "var(--accent)" }}>Research Agent</span>
      </div>

      {(status === "idle" || status === "error") && (
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 800, color: "var(--bright)", marginBottom: 8, letterSpacing: "-0.3px" }}>Intake form</h2>
          <p style={{ fontSize: 12, color: "var(--dim)", marginBottom: 26, lineHeight: 1.6 }}>Fill the fields below. The agent returns its analysis in about 45 seconds.</p>

          <div style={{ display: "flex", flexDirection: "column", gap: 18, marginBottom: 24 }}>
            {FIELDS.map(({ key, label, hint, rows }) => (
              <div key={key}>
                <label style={{ display: "block", fontSize: 9, letterSpacing: "2px", textTransform: "uppercase", color: "var(--dim)", marginBottom: 8, fontWeight: 700 }}>{label}</label>
                {rows > 0 ? (
          ...
          {error && (
            <div style={{ ...card, padding: "14px 16px", marginBottom: 16, color: "#d05050", fontSize: 12 }}>
              {error}
            </div>
          )}
      ...
      {status === "running" && (
        <div style={{ textAlign: "center", padding: "90px 0" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 12, color: "var(--accent)" }}>
            ...
            <span style={{ fontSize: 13, letterSpacing: "3px", textTransform: "uppercase" }}>Working {elapsed}s</span>
          </div>
          <p style={{ fontSize: 12, color: "var(--dim)", marginTop: 18 }}>Analyzing project, market and competitors...</p>
        </div>
      )}
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22, flexWrap: "wrap", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              ...
              <span style={{ fontSize: 12, color: "var(--green)", fontWeight: 600 }}>Done in {elapsed}s</span>
            </div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              {meta && (
                <span style={{ fontSize: 10, padding: "6px 12px", borderRadius: 9, background: "var(--card)", boxShadow: "var(--nm-out-sm)", color: "var(--dim)" }}>
                  {meta.tokens.toLocaleString()} tokens / ${meta.costUsd.toFixed(4)}
                </span>
              )}
              <button onClick={() => { setStatus("idle"); setResult(null); }} style={{ fontSize: 10, padding: "6px 14px", borderRadius: 9, background: "var(--card)", boxShadow: "var(--nm-out-sm)", color: "var(--text)", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>
                New
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {Object.entries(result).map(([key, val]) => {
              const accent = key === "redFlags" ? "#d05050" : key === "opportunities" ? "var(--green)" : "var(--accent)";
              return (
                <div key={key} style={{ ...card, overflow: "hidden" }}>
                  <div style={{ padding: "12px 18px", display: "flex", alignItems: "center", gap: 9 }}>
                    <div style={{ width: 7, height: 7, borderRadius: "50%", background: accent }} />
                    <span style={{ fontSize: 10, letterSpacing: "1.5px", textTransform: "uppercase", fontWeight: 700, color: accent }}>
                      {key.replace(/([A-Z])/g, " $1")}
                    </span>
                  </div>
                  <div style={{ padding: "0 18px 16px", fontSize: 12, color: "var(--text)", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
